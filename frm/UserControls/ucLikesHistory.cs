...
{
  public class ucLikesHistory : UserControl
  {
    private readonly PostService _postService = new PostService();
    private FlowLayoutPanel flpLikes = null!;
    private Label lblTitle = null!;

    public Guid CurrentUserId { get; set; }
    public event EventHandler<int>? OnOpenPostRequested;

    public ucLikesHistory(Guid currentUserId)
    {
    ...
    private void InitializeComponent()
    {
      ...
      lblTitle = new Label
      {
        Text = "Your Likes History",
        AutoSize = true,
        Font = new Font("Segoe UI", 16, FontStyle.Bold),
        Location = new Point(20, 20)
      };

      flpLikes = new FlowLayoutPanel
      {
      ...
      Controls.Add(flpLikes);
      Resize += (_, __) => { flpLikes.Size = new Size(Width - 40, Height - 90); };
    }

    private void LoadHistory()
    {
      flpLikes.Controls.Clear();
      var likedPosts = _postService.GetPostsLikedByUser(CurrentUserId)
      ...
      {
        flpLikes.Controls.Add(BuildLikePanel(post));
      }
    }

    private Panel BuildLikePanel(Post post)
    {
      var p = new Panel
      {
        Width = flpLikes.Width - 30,
        Height = 100,
      ...
      {
        AutoSize = false,
        Text = $"Post #{post.PostID}: {Truncate(post.Content, 60)}",
        Dock = DockStyle.Top,
        Height = 30,
      ...
      {
        AutoSize = false,
        Text = $"Created: {post.CreatedAt:yyyy-MM-dd HH:mm}",
      ...
      var btnOpen = new RoundedButton
      {
        ...
        Width = 110,
        Height = 32,
        Location = new Point(p.Width - 130, 58),
        Anchor = AnchorStyles.Top | AnchorStyles.Right,
        BorderRadius = 18
      };
      btnOpen.Click += (_, __) => OnOpenPostRequested?.Invoke(this, post.PostID);

      p.Controls.Add(btnOpen);
      p.Controls.Add(lblMeta);
      p.Controls.Add(lblPost);
      p.Resize += (_, __) => btnOpen.Location = new Point(p.Width - 130, 58);

      StyleLikePanel(p, lblPost, lblMeta, btnOpen);
      return p;
    }

    private void StyleLikePanel(Panel p, Label lblPost, Label lblMeta, RoundedButton btnOpen)
    {
      ...
      foreach (Control c in flpLikes.Controls)
      {
        if (c is Panel p)
        {
          ...
          if (btn != null && labels.Count >= 2)
          {
            ...
            StyleLikePanel(p, lblPost, lblMeta, btn);
          }
        }
      }
    }
