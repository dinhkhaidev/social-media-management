...
namespace SocialManager.frm.UserControls
{
    public class ucReportManagement : UserControl
    {
        ...
        private Button btnHideContent = null!;
        private Button btnBanUser = null!;
        ...
        private Panel CreateSearchFilterPanel()
        {
            ...
            cmbStatusFilter.Items.AddRange(new[] { "Tất cả", "New", "In Review", "Pending Action", "Resolved", "Escalated", "Rejected" });
        ...
        private Panel CreateDetailHeader(Report report)
        {
            ...
            // Use FlowLayoutPanel for better layout control
            var flowPanel = new FlowLayoutPanel
            {
                Dock = DockStyle.Fill,
            ...
            {
                Text = $"👤 Người báo cáo: {reporter?.UserName ?? "Unknown"}",
                Font = new Font("Segoe UI", 9.5F),
                ForeColor = Color.White,
                AutoSize = true,
                Margin = new Padding(0, 0, 0, 5)
            ...
            var lblReported = new Label
            {
                Text = $"⚠️ Người bị báo cáo: {reported?.UserName ?? "Unknown"}",
                Font = new Font("Segoe UI", 9.5F),
            ...
            {
                Text = $"🕐 {report.ReportedAt:dd/MM/yyyy HH:mm}",
                Font = new Font("Segoe UI", 9F),
            ...
            var lblStatus = new Label
            {
                Text = $"📊 Trạng thái: {GetStatusText(report.Status)}",
                Font = new Font("Segoe UI", 9F, FontStyle.Bold),
                ForeColor = Color.White,
                BackColor = GetStatusColor(report.Status),
                AutoSize = true,
                Padding = new Padding(8, 4, 8, 4),
                Margin = new Padding(0, 5, 0, 0)
            };
            
            flowPanel.Controls.AddRange(new Control[] { lblReportID, lblType, lblReporter, lblReported, lblDate, lblStatus });
            panel.Controls.Add(flowPanel);
            return panel;
        }
        ...
        private Panel CreateDetailActionsPanel(Report report)
        {
            ...
            var btnMarkInReview = new Button
            {
                Text = "🔍 Đang xem xét",
                Width = 120,
                Height = 35,
        ...
        private void BtnHideContent_Click(object? sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                {
                    // Hide content and update post/comment in database
                    if (report.ReportType == "Post")
                    {
                        var posts = postService.GetAllPosts(true);
                        var post = posts.FirstOrDefault(p => p.PostID == report.ContentID);
                        if (post != null)
                        {
                            // Use SoftDeletePost to mark as deleted
                            postService.SoftDeletePost(report.ContentID);
                    ...
                    else if (report.ReportType == "Comment")
                    {
                        ...
                        if (comment != null)
                        {
                            // Use SoftDeleteComment to mark as deleted
                            commentService.SoftDeleteComment(report.ContentID);
                        ...
                        "Đang thực hiện hành động ẩn nội dung", "Processing"))
                    {
                        ...
                        if (reportService.UpdateReportStatus(report.ReportID, "Resolved", currentUserID, 
                            "Đã hoàn thành - Nội dung đã bị ẩn và tăng violation count", "Content Hidden"))
                        {
                            successCount++;
                        }
                    }
        ...
        private void BtnBanUser_Click(object? sender, EventArgs e)
        {
            ...
            // Get user report counts to show warning information
            var userReportInfo = new List<string>();
            int willBeBanned = 0;
            int willBeWarned = 0;
            ...
            foreach (var userID in reportedUserIDs)
            {
                ...
                if (user != null)
                {
                    int currentViolations = user.ViolationCount + 1; // +1 vì sắp tăng
                    int currentReports = userService.GetReportCount(userID);
                    
                    if (currentViolations >= 3)
                    {
                        willBeBanned++;
                        userReportInfo.Add($"• {user.UserName}: {currentViolations} vi phạm ({currentReports} báo cáo) → BỊ CẤM");
                    }
                    else if (currentViolations >= 1)
                    {
                        willBeWarned++;
                        userReportInfo.Add($"• {user.UserName}: {currentViolations} vi phạm ({currentReports} báo cáo) → CẢNH BÁO");
                    }
                }
            }
            
            var result = MessageBox.Show(
                $"Xác nhận vi phạm cho {reportedUserIDs.Count} người dùng?\n\n" +
                $"📊 Kết quả dự kiến:\n" +
                $"   🚫 Bị cấm: {willBeBanned} người (3+ vi phạm)\n" +
                $"   ⚠️ Cảnh báo: {willBeWarned} người (1-2 vi phạm)\n\n" +
                (userReportInfo.Count > 0 && userReportInfo.Count <= 5 
                    ? $"Chi tiết:\n{string.Join("\n", userReportInfo.Take(5))}\n\n" 
                    : "") +
                $"💡 Lưu ý: Hành động này sẽ tăng ViolationCount (vi phạm được xác nhận)\n" +
                $"ReportCount chỉ để thống kê, không ảnh hưởng trạng thái tài khoản.",
            ...
            if (result == DialogResult.Yes)
            {
                ...
                foreach (var userID in reportedUserIDs)
                {
                    // Tăng ViolationCount - điều này sẽ tự động cập nhật StatusId
                    if (userService.IncrementViolationCount(userID))
                    {
                        successCount++;
                        var user = userService.GetUserById(userID);
                        if (user != null)
                        {
                            if (user.StatusId == -1) bannedCount++;
                            else if (user.StatusId == 2) warnedCount++;
                        }
                    }
                }
                ...
                var reportIDs = selectedReports.Select(r => r.ReportID).ToList();
                reportService.BatchUpdateReports(reportIDs, "Escalated", currentUserID, "Violation confirmed by Admin");
                
                MessageBox.Show(
                    $"✅ Đã xử lý {successCount} người dùng!\n\n" +
                    $"📊 Kết quả:\n" +
                    $"   🚫 Bị cấm: {bannedCount} người (3+ vi phạm)\n" +
                    $"   ⚠️ Cảnh báo: {warnedCount} người (1-2 vi phạm)\n\n" +
