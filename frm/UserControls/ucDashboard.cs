    ...
    public partial class ucDashboard : UserControl
    {
        ...
        private void LoadDashboardData()
        {
            try
            {
                ...
                // Initialize platform combo box safely
                ...
                if (lstRecentActivity != null)
                {
                    ...
                    lstRecentActivity.Items.AddRange(new object[] {
                        "? New post published on Facebook",
                        "?? 5 new comments on Instagram", 
                        "?? Post scheduled for tomorrow at 9 AM",
        ...
        private void btnPostNow_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtPostContent == null || cmbPlatform == null)
                {
                    MessageBox.Show("Post composer is not available.", "Error", 
                        MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                ...
                MessageBox.Show($"Post published to {platform} successfully!", "Success", 
        ...
        private void btnSchedule_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtPostContent == null || cmbPlatform == null || dtpSchedule == null)
                {
                    MessageBox.Show("Scheduler is not available.", "Error", 
                        MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                ...
                DateTime scheduleTime = dtpSchedule.Value;
                if (scheduleTime <= DateTime.Now)
                {
                    MessageBox.Show("Schedule time must be in the future.", "Validation Error", 
                        MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                string platform = cmbPlatform.Text;
                MessageBox.Show($"Post scheduled for {platform} at {scheduleTime:MM/dd/yyyy HH:mm}!", "Success", 
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
