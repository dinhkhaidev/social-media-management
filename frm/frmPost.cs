...
namespace SocialManager.frm
{
  public partial class frmPost : Form
  {
    ...
    private void InitializeForm()
    {
      ...
      UIHelper.ApplyRoundedCorners(pnlMain, 15);
      // txtContent is RichTextBox, skip for now (no overload available)
      UIHelper.ApplyRoundedCorners(btnSelectImage, 8);
      UIHelper.ApplyRoundedCorners(btnPost, 10);
      UIHelper.ApplyRoundedCorners(btnCancel, 10);
      UIHelper.ApplyRoundedCorners(pnlImagePreview, 12);
      UIHelper.ApplyRoundedCorners(btnRemoveImage, 8);
    }
    ...
    {
      // Kiểm tra xem tài khoản có bị hạn chế đăng bài không
      var currentUser = AuthSessionService.CurrentUser;
      if (currentUser != null)
      {
        // Lấy report count từ ReportService
        var userService = new UserService();
        int reportCount = userService.GetReportCount(currentUser.UserID);
        currentUser.ReportCount = reportCount; // Sync

        if (!currentUser.CanPost)
        {
          MessageBox.Show(
              $"Tài khoản của bạn đã bị tạm ngừng quyền đăng bài do vi phạm {currentUser.ReportCount} lần.\n" +
              $"Trạng thái: {currentUser.AccountStatus}\n\n" +
      ...
      try
      {
        ...
        // Check if postService is initialized
        if (postService == null)
        {
          MessageBox.Show("Lỗi khởi tạo dịch vụ. Vui lòng thử lại.", "Lỗi",
